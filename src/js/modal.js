...
function focusTrap(modalEl) {
  ...
  const getNodes = () =>
    Array.from(modalEl.querySelectorAll(sel)).filter(el => !el.hasAttribute('inert'));
  function onKey(e) {
    if (e.key !== 'Tab') return;
...
async function ensureCss(type) {
  ...
  const existing = [...document.querySelectorAll('link[rel="stylesheet"]')].find(
    l => l.getAttribute('href') === href || l.dataset.modalStyle === type
  );
  if (existing) {
    STYLE_REGISTRY.set(type, existing);
    return existing;
  }
...
function renderContent(bodyEl, content, props, api) {
  ...
  if (nodeOrHtml instanceof Node) bodyEl.appendChild(nodeOrHtml);
    ...
    bodyEl.innerHTML = nodeOrHtml; // use apenas suas strings
...
export function open(options = {}) {
  ...
  const { root, dialog, backBtn, closeBtn, body } = buildModalDOM(
    { title, showBack, showClose, size, iconColor },
    api
  );
  ...
  function keyHandler(e) {
    if (e.key === 'Escape' && escToClose) close('escape');
  }
  ...
  // ---- API exposta ----
  function setTitle(t) {
    dialog.querySelector('.modal-title').textContent = t ?? '';
  }
  function setContent(c) {
    renderContent(body, c, props, api);
  }
  function setBack(fn) {
    backHandler = typeof fn === 'function' ? fn : null;
  }
  function setBackTo(type, p = {}) {
    setBack(() => openModal({ type, props: p }));
  }
  function swap(type, p = {}) {
    return openModal({ type, props: p });
  }
...
export async function openModal({ type, props = {}, overrides = {} } = {}) {
  ...
  const [{ render, meta }] = await Promise.all([ensureDefined(type), ensureCss(type)]).then(
    ([reg]) => [reg]
  );
