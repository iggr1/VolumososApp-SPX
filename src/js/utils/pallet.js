...
function scheduleUpdateCounts(delayMs = 80, options = {}) {
  ...
  if (forceRemote !== undefined)
    pendingUpdateOptions.forceRemote = pendingUpdateOptions.forceRemote || forceRemote;
...
export function verifyAlreadyInLocalPallet(brCode) {
  ...
  return current.some(
    item =>
      String(item.brCode || item.brcode || '')
        .trim()
        .toUpperCase() === code
  );
}
...
export function addIntoQueue(options = {}) {
  ...
  const norm = packages
    .map(p => ({
      brCode: String(p.brCode || p.brcode || '')
        .trim()
        .toUpperCase(),
  ...
  try {
    ...
    console.error('getConfigs falhou:', err);
  }
...
export async function deletePallets(ids) {
  ...
  const payload = list.map(n => Number(n)).filter(n => Number.isFinite(n) && n > 0);
    ...
    .map(g => {
      ...
      <div class="line">
        ...
        <span class="route">${esc(p.route)}</span>
          ...
          .join('') || `<div class="line line--empty">— sem pacotes —</div>`;
